...
class PennylaneQNNCircuit:
    def __init__(self, enc=1, pqc=1, meas=1, layers=1, qubit=1):
        ...
        self.qubit = qubit
        self.layers = layers
        self.pqc_builder = ParametricCircuitsPennylane(
            pqc=self.pqc, qubit=self.qubit, layers=self.layers)
        self.enc_builder = EncodingCircuitsPennylane(
            enc=self.enc, qubit=self.qubit)
        self.meas_builder = MeasurementCircuitsPennylane(
            meas=self.meas, qubit=self.qubit)
